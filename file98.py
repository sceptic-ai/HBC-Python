...
class Blockchain:
    ...
    def save_data(self):
        ...
        try:
            with open("resources/blockchain_{}.txt".format(self.node_id), mode="w") as f:
                ...
                # }
                # f.write(pickle.dumps(save_data_format))
        except IOError:
            print("Saving failed!")
    ...
    def add_transaction(self, recipient, sender, signature, amount=1.0, is_receiving=False):
        ...
        :argument amount: The amount of the coins sent with the transaction (default = 1.0)
        """
        # transaction = {
        #     "sender": tx_sender,
        ...
        for incometx in block["transactions"]:
            for opentx in stored_transactions:
                if opentx.sender == incometx["sender"] and opentx.recipient == incometx["recipient"] \
                        and opentx.signature == incometx["signature"] and opentx.amount == incometx["amount"]:
                    try:
    ...
    def remove_peer_node(self, node):
        """
        Removes a new node form the peer node set.
